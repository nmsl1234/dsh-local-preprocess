...
test('multibyte reason passthrough', async () => {
  const fetchImpl = async () => ({ status: 200, json: async () => ({ message: { content: '已脱敏' } }) })
  const r = await preprocessText('秘密', 'i', { provider: 'ollama', baseUrl: 'x', model: 'm' }, null, fetchImpl)
  assert.equal(r.changed, true); assert.equal(r.text, '已脱敏')
})
